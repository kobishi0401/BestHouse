...
本文件以「使用者與交付對象」為中心，完整說明 BestHouse 網站可提供的功能、各技術使用位置與目的、操作方式，以及目前限制與後續擴充方向。  
...
BestHouse 是一個「房地產展示型網站」前端專案，核心目標是：
...
- 無後端 API：表單不會真正送出到資料庫或 CRM
- 目前房源資料為靜態寫死於 HTML，非動態資料來源
- 部分「預約賞屋」按鈕仍為佔位連結（`#`）
- 字型依賴 Google Fonts，網路受限時會 fallback

---

## 8) 建議下一步（若要進入正式商用）

1. 串接後端表單 API（寄信/建單/CRM）
2. 將房源改為資料庫驅動（可後台管理）
3. 增加 SEO 與社群分享 metadata（Open Graph）
4. 增加 GA4 / 行為追蹤與轉換事件
5. 增加圖片壓縮與 lazy loading，提升載入速度
...
- [ ] 表單送出可顯示成功提示
- [ ] 手機版選單可展開與收合
- [ ] 主要瀏覽器（Chrome/Edge/Safari）可正常瀏覽

---

如需，我可以在下一步幫你再做一版「資方簡報版 README（更商務語氣、較少技術術語）」或「開發者版 README（更技術細節與維護流程）」雙版本文件。
